import {
  ...
  nsd74 as nsd74Str,
  nsd81 as nsd81Str,
} from '@openenergytools/scl-lib/dist/foundation/codecomponents/nsds.js';

const nsd74 = new DOMParser().parseFromString(nsd74Str, 'application/xml');
const nsd73 = new DOMParser().parseFromString(nsd73Str, 'application/xml');
const nsd72 = new DOMParser().parseFromString(nsd72Str, 'application/xml');
const nsd81 = new DOMParser().parseFromString(nsd81Str, 'application/xml');
...
export function initializeNsdoc(): Nsdoc {
  ...
  function getDODataDescription(element: Element): { label: string } {
    ...
    const lnClass = nsd74.querySelector(
      `NS > LNClasses > LNClass[name="${element.parentElement?.getAttribute('lnClass')}"]`,
    );
  ...
  function getSDODataDescription(element: Element): { label: string } {
    ...
    const subDataObject = nsd73.querySelector(
      `CDCs > CDC[name="${element.parentElement?.getAttribute('cdc')}"] > SubDataObject[name="${sdoName}"]`,
    );
  ...
  function getBDADataDescription(
    ...
    const serviceDataAttr = nsd81.querySelector(
      `ServiceConstructedAttributes > ServiceConstructedAttribute[name="${daParent!.getAttribute('name')}"]`,
    );
    ...
    if (serviceDataAttr) {
      ...
      const dataAttrParent =
        nsd73.querySelector(`NS > CDCs > CDC[name="${daParent!.closest('DOType')?.getAttribute('cdc')}"] >
  ...
  function getSubDataAttribute(
    ...
    const subDataAttr = nsd73.querySelector(
      `ConstructedAttributes > ConstructedAttribute[name="${parent?.getAttribute('type')}"] > SubDataAttribute[name="${bdaElementName}"]`,
    );
    return (
      subDataAttr ??
      getSubDataAttribute(
        nsd73.querySelector(
          `ConstructedAttributes > ConstructedAttribute[name="${parent?.getAttribute('type')}"] > SubDataAttribute`,
        )!,
        bdaElementName,
      )
    );
